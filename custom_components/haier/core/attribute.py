...
class V1SpecAttributeParser(HaierAttributeParser, ABC):
    ...
    _EXCLUDED_ATTRIBUTE_NAMES = {
        'prtlVersion',
        'curveDownloadSuccess',
        'curveManualSetSuccess',
        'ratedPower',
        'targetRatedPower',
        'dateYY',
        'dateMM',
        ...
        'scourTargetTemp',
        'curveID',
        'altitudeAdaptationStatus',
        'timingCycleStartTime',
        'timingCycleEndTime',
        'timingCycleStartDate',
        'timingCycleEndDate',
        'cycFuncSelect',
        'zeroColdWaterFuncExist',
    }

    def parse_attribute(self, attribute: dict) -> HaierAttribute:
        name = attribute.get('name', '')
        if name in self._EXCLUDED_ATTRIBUTE_NAMES or name in self._GLOBAL_ATTRIBUTES:
            return None

        if 'value' not in attribute:
            return None

        writable = attribute.get('writable', False)

        if not writable and attribute.get('readable', True):
            return self._parse_as_sensor(attribute)

        if writable and equals_ignore_case(attribute['valueRange']['type'], 'STEP') and contains_any_ignore_case(attribute['valueRange']['dataStep']['dataType'], ['Integer', 'Double']):
            return self._parse_as_number(attribute)

        if writable and V1SpecAttributeParser._is_binary_attribute(attribute):
            return self._parse_as_switch(attribute)

        if writable and equals_ignore_case(attribute['valueRange']['type'], 'LIST'):
    ...
    def parse_global(self, attributes: List[dict]):
        ...
        # 燃气热水器
        if 'outWaterTemp' in all_attribute_keys and 'targetTemp' in all_attribute_keys and 'onOffStatus' in all_attribute_keys:
